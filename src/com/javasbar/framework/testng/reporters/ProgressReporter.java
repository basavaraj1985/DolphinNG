...
public class ProgressReporter implements ITestListener
{
    ...
    public ProgressReporter()
    {
        ...
        {
            LOG.info("Example : error : cannot find template " + templateFile);
        } catch (ParseErrorException pee)
        {
            LOG.info("Example : Syntax error in template " + templateFile + ":" + pee);
        } catch (Exception e)
        {
            e.printStackTrace();
        }
    ...
    private void update()
    {
        ...
        {
//			e1.printStackTrace();
            LOG.info("Ignorable error: " + e1.getMessage());
        } finally
        {
            if (null != writer)
            {
